...
public class VCO : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // vol reads straight from knob — 0 by default until user turns it
        float vol = volKnob != null ? volKnob.Value01 : 0f;
        if (volSlot != null && volSlot.OccupiedBy != null)
            vol = Mathf.Clamp01(PatchRouter.Instance.ReadValue(volSlot.SlotId));

        // rm reads straight from knob — defaults to 1 since it's a modulator not a volume
        ...
        if (debugFrame % 60 == 0)
            Debug.Log($"[VCO] freq={freq:F1} vol={vol:F3} output={output:F4} outSlot={outSlot?.SlotId}");
        ...
        // vol2 also defaults to 0
        float vol2 = vol2Knob != null ? vol2Knob.Value01 : 0f;
